...
(function (document, $, wordfind) {
    ...
    */
    var WordFindGame = function() {
        ...
        */
        var drawWords = function (el, words, defs) {
      
            var output = '<ul>';
            for (var i = 0, len = words.length; i < len; i++) {
                var word = words[i];
                var def = defs[i];
                output += '<li class="word ' + word + '">' + def;
            }
            output += '</ul>';
        ...
        */
        ...
        return {
            ...
            */
            create: function(words, definitions, puzzleEl, wordsEl, options) {
        
                wordList = words.slice(0);//.sort();
                defList = definitions.slice(0);//.sort();

                var puzzle = wordfind.newPuzzle(words, options);

                // draw out all of the words
                drawPuzzle(puzzleEl, puzzle);
                drawWords(wordsEl, wordList, defList);
